...
位置情報インターバル共有アプリ。主催者がセッションを作成し「開示インターバル」と「期限」を設定する。参加者の位置は**継続的に取得されつつ、設定間隔ごとにのみ**グループへ開示され、期限が来ると追跡は完全に停止する。用途は家族の見守り・イベント運営・登山/マラソンの応援など。
...
- **追跡は必ず有限**: すべてのセッションは `expires_at` で自動終了する。無期限モードは作らない。
