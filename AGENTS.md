...
- **M2 の中核実装済み**: 匿名デバイス認証、セッション作成/参加/終了、位置バッチアップロード、開示ビュー、毎分 Cron の開示・期限処理。不変条件のユニットテスト・統合テストあり。無応答アラートと開示プッシュは未実装。
- モバイルはセッション作成→参加→地図表示（開示カウントダウン付き）まで動線あり。UI の @expo/ui 化と磨きは M6。
